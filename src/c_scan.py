class CScan:
    ...
    def split_requests(self, look: bool):
        ...
        if self.initial_direction == "inc":
            self.requests_greater_or_equal = [
                request
            ...
            self.requests_less_than = [
                request
                for request in self.request_queue
                if request < self.initial_position
            ]
            self.request_queue = (
                self.requests_greater_or_equal + self.requests_less_than
            )
        else:
            self.requests_less_than_or_equal = [
                request
                for request in self.request_queue
                if request <= self.initial_position
            ]
            ...
            self.requests_greater = [
                request
                for request in self.request_queue
                if request > self.initial_position
            ]
            self.request_queue = (
                self.requests_less_than_or_equal + self.requests_greater
            )
    ...
    def run(self, look: bool):
        ...
        self.logger.info(
            f"C-SCAN {'with look' if look else 'without look'} - Order of service: {self.request_queue}"
        )
        self.logger.info(
            f"C-SCAN {'with look' if look else 'without look'} - Total steps = {total_movement}"
        )
